import reducer from "./starredRepositoriesReducer";
import { StarRepositoryActionType } from "../action-types";
import { StarredRepositoriesState } from "./starredRepositoriesReducer";
import { Repository } from "../../shared/apis/github/types";

test("initial state", () => {
  const initialState: StarredRepositoriesState = {};

  expect(reducer(undefined, undefined)).toEqual(initialState);
});

test("starring a repository", () => {
  const expectedState = { [mockRepository.id]: mockRepository };

  expect(
    reducer(undefined, {
      type: StarRepositoryActionType.REPOSITORY_STARRED,
      payload: { [mockRepository.id]: mockRepository },
    })
