...
test("should be in loading state", () => {
  const loading: RepositoriesState = {
    loading: true,
    error: null,
    data: { total_count: 0, incomplete_results: true, items: [] },
  };
...
test("repository search results", () => {
  const baseResult: RepositoriesState = {
    loading: false,
    error: null,
    data: { total_count: 1, incomplete_results: true, items: [mockRepository] },
  };
